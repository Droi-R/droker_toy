    ...
    public static void w(String _tag, String msg) {
        try {
            if (debug) {
                Exception e = new Exception();
                StackTraceElement[] element = e.getStackTrace();
                Log.w(_tag, "(" + element[1].getFileName() + ":" + element[1].getLineNumber() + ")" + "\t" + element[1].getMethodName() + " -> " + msg);
            }
        } catch (Exception ignore) {
        }
    }
...
//                Timber.tag(tag).d(new GsonBuilder().setPrettyPrinting().create().toJson(new JsonParser().parse(msg)));
    ...
    public static void dWebsocket(String tag, String tag2, String msg) {
        try {
            if (debug) {
                Exception e = new Exception();
                StackTraceElement[] element = e.getStackTrace();
